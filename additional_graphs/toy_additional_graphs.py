...
class ToyAdditionalGraphs:
    ...
    def __init__(self):
        g1 = HypergraphModel()
        g1.add_vertices(np.array(["a", "b", "target"]), type="entities")
        g1.add_vertices(np.array(["e"]), type="events")
        g1.populate_discovered(type="entities")
        g1.populate_discovered(type="events")
        g1.append_edges(np.array([["a", "r1", "e"], ["b", "r2", "e"], ["target", "r3", "e"]]), sources="entities", targets="events")
    ...
    def produce_additional_graphs(self):
        for hypergraph, mapping, target in zip(self.hypergraphs, self.mappings, self.targets):
            yield hypergraph, mapping, target